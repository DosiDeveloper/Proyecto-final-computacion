        ...
        with col_g1:
            if rating_filter != []:
                ...
                fig1 = px.funnel(y=only_cat_selected.cat.remove_unused_categories().unique().tolist(), 
                                 x=only_cat_selected.cat.reorder_categories(rating_filter).value_counts()[:len(rating_filter)],
...
def graph_tab(filtered_df: pd.DataFrame):
    with st.container():
        ...
        if len(select_dist_item) == 1:
            ...
            fig.update_layout(
                title_text=f"Distribucion de {select_dist_item[0]}", xaxis_title_text=select_dist_item[0], showlegend=False)
        ...
        for item in select_dist_item:
            ...
            fig1.update_layout(
                title_text=f"Distribucion de {item}", xaxis_title_text=item, showlegend=False)
